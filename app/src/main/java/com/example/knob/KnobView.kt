...
class KnobView : View {
    ...
    /**
     * Paint used to draw the knob
     * */
    private var mKnobPaint: Paint? = null

    /**
     * Paint use to draw circles outside the knob's main view
     * */
    private var mMarkerCircle: Paint? = null

    /**
     * Paint use to draw selected circle outside the knob's main view
     * */
    private var mSelectedMarkerCircle: Paint? = null
     ...
     */
    private var mCircleStyle: Paint.Cap? = null

    /**
     * current in negative half cycle.
     */
    private var mIsInNegativeHalf: Boolean = false
     ...
     */
    private var mCircleXRadius: Float = 0.toFloat()
     ...
     */
    private var mPointerStrokeWidth: Float = 0.toFloat()

    /**
     * The width of the pointer halo (in pixels).
     */
    private var mPointerHaloWidth: Float = 0.toFloat()

    /**
     * The width of the pointer halo border (in pixels).
     */
    private var mPointerHaloBorderWidth: Float = 0.toFloat()
     ...
     */
    private var mPointerAngle: Float = 0.toFloat()

    /**
     * Start angle of the KnobView.
    ...
    /**
     * End angle of the KnobView.
     * Note: If mStartAngle and mEndAngle are set to the same angle, 0.1 is subtracted
     * from the mEndAngle to make the circle function properly.
     */
    private var mEndAngle: Float = 0.toFloat()

    private var angles = arrayOfNulls<Float>(8)
    private var markerPositions = ArrayList<Pair<Float, Float>>()
    ...
    /**
     * `RectF` that represents the marker circles
     */
    val pathMarker = RectF()
    ...
    private var mKnobColor = DEFAULT_KNOB_COLOR
     ...
     */
    private var mTotalCircleDegrees: Float = 0.toFloat()

    /**
     * Distance (in degrees) that the current progress makes up in the circle.
     */
    private var mProgressDegrees: Float = 0.toFloat()
    ...
    private var mMarkerCirclePath: Path? = null

    /**
     * Max value that this KnobView is representing.
     */
    private var mMax: Float = 0.toFloat()

    /**
     * Progress value that this KnobView is representing.
     */
     ...
     * If false, then the View itself determines the size of the KnobView.
     */
    private var mCustomRadii: Boolean = false
     ...
     */
    private var mMaintainEqualCircle: Boolean = false
     ...
     */
    private var mMoveOutsideCircle: Boolean = false
     ...
     */
    private var mHideProgressWhenEmpty: Boolean = false

    /**
     * When the user is touching the circle on ACTION_DOWN, this is set to true.
     * Used when touching the KnobView.
     */
    ...
    /**
     * Get the progress of the KnobView.
     * @return The progress of the KnobView.
     */
    /**
     * Set the progress of the KnobView.
     * If the progress is the same, then any listener will not receive a onProgressChanged event.
     * @param progress The progress to set the KnobView to.
     */
    var progress: Float
        get() {
    ...
    /**
     * Get the current max of the KnobView.
     * @return Synchronized integer value of the max.
     */
    /**
     * Set the max of the KnobView.
     ...
     * @param max The new max for the KnobView.
     */
    ...
    /**
     * Initialize the KnobView with the attributes from the XML style.
    ...
    private fun initAttributes(attrArray: TypedArray) {
        ...
        mCircleYRadius =
            attrArray.getDimension(R.styleable.cs_CircularSeekBar_cs_circle_y_radius, DEFAULT_CIRCLE_Y_RADIUS)
        mPointerStrokeWidth =
            attrArray.getDimension(R.styleable.cs_CircularSeekBar_cs_pointer_stroke_width, DEFAULT_POINTER_STROKE_WIDTH)
        mPointerHaloWidth =
            attrArray.getDimension(R.styleable.cs_CircularSeekBar_cs_pointer_halo_width, DEFAULT_POINTER_HALO_WIDTH)
        ...
        mCircleStrokeWidth =
            attrArray.getDimension(R.styleable.cs_CircularSeekBar_cs_circle_stroke_width, DEFAULT_CIRCLE_STROKE_WIDTH)
        ...
        mPointerHaloColor =
            attrArray.getColor(R.styleable.cs_CircularSeekBar_cs_pointer_halo_color, DEFAULT_POINTER_HALO_COLOR)
        ...
        mCircleProgressColor =
            attrArray.getColor(R.styleable.cs_CircularSeekBar_cs_circle_progress_color, DEFAULT_CIRCLE_PROGRESS_COLOR)
        ...
        mPointerAlphaOnTouch =
            attrArray.getInt(R.styleable.cs_CircularSeekBar_cs_pointer_alpha_ontouch, DEFAULT_POINTER_ALPHA_ONTOUCH)
        ...
        mProgress = attrArray.getInt(R.styleable.cs_CircularSeekBar_cs_progress, DEFAULT_PROGRESS).toFloat()
        mCustomRadii =
            attrArray.getBoolean(R.styleable.cs_CircularSeekBar_cs_use_custom_radii, DEFAULT_USE_CUSTOM_RADII)
        mMaintainEqualCircle =
            attrArray.getBoolean(R.styleable.cs_CircularSeekBar_cs_maintain_equal_circle, DEFAULT_MAINTAIN_EQUAL_CIRCLE)
        mMoveOutsideCircle =
            attrArray.getBoolean(R.styleable.cs_CircularSeekBar_cs_move_outside_circle, DEFAULT_MOVE_OUTSIDE_CIRCLE)
        isLockEnabled = attrArray.getBoolean(R.styleable.cs_CircularSeekBar_cs_lock_enabled, DEFAULT_LOCK_ENABLED)
        mDisablePointer =
            attrArray.getBoolean(R.styleable.cs_CircularSeekBar_cs_disable_pointer, DEFAULT_DISABLE_POINTER)
        isNegativeEnabled =
            attrArray.getBoolean(R.styleable.cs_CircularSeekBar_cs_negative_enabled, DEFAULT_NEGATIVE_ENABLED)
        mIsInNegativeHalf = false
        mDisableProgressGlow =
            attrArray.getBoolean(R.styleable.cs_CircularSeekBar_cs_disable_progress_glow, DEFAULT_DISABLE_PROGRESS_GLOW)
        ...
        mEndAngle =
            (360f + attrArray.getFloat(R.styleable.cs_CircularSeekBar_cs_end_angle, DEFAULT_END_ANGLE) % 360f) % 360f
    ...
    private fun initPaints() {
        ...
        mKnobPaint = Paint()
        mKnobPaint?.isAntiAlias = true
        mKnobPaint?.isDither = true
        mKnobPaint?.color = mKnobColor
        mKnobPaint?.style = Paint.Style.FILL

        mMarkerCircle = Paint()
        mMarkerCircle?.isAntiAlias = true
        mMarkerCircle?.isDither = true
        mMarkerCircle?.color = DEFAULT_MARKER_CIRCLE
        mMarkerCircle?.strokeWidth = DEFAULT_MARKER_STROKE_WIDTH
        mMarkerCircle?.style = Paint.Style.STROKE

        mSelectedMarkerCircle = Paint()
        mSelectedMarkerCircle?.isAntiAlias = true
        mSelectedMarkerCircle?.isDither = true
        mSelectedMarkerCircle?.color = DEFAULT_POINTER_COLOR
        mSelectedMarkerCircle?.strokeWidth = DEFAULT_MARKER_STROKE_WIDTH

        mSelectedMarkerCircle?.style = Paint.Style.FILL_AND_STROKE
    ...
    private fun resetRects() {
        pathCircle.set(-mCircleWidth, -mCircleHeight, mCircleWidth, mCircleHeight)
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawCircle(0f, 0f, mCircleWidth - circleStrokeWidth / 2, mKnobPaint!!)
    ...
    private fun setProgressBasedOnAngle(angle: Float) {
        ...
        val progressIndex = progressMilestones.findClosest(mProgress)
        selectedmarkerIndex = progressIndex.first
        mProgress = progressIndex.second
    }
    ...
    val progressMilestones = ArrayList<Float>()
    private fun calculateMarkerPositions() {
        var angleSeparation = 30
        markerPositions.clear()
        progressMilestones.clear()

/*
* abs(DEFAULT_START_ANGLE -90)
* */
        markerPositions.add(
            Pair(
                sin(Math.toRadians((360 - 60).toDouble())).toFloat() * (mCircleWidth + PADDING_MARKER_PROGRESS_CIRCLE),
                cos(Math.toRadians((360 - 60).toDouble())).toFloat() * (mCircleWidth + PADDING_MARKER_PROGRESS_CIRCLE)
            )
        )
        // mMax * mProgressDegrees / mTotalCircleDegrees
        progressMilestones.add(mMax * (300 - 60) / mTotalCircleDegrees)
        for (i in 1..7) {
            markerPositions.add(
                Pair(
                    Math.sin(Math.toRadians(360 - (60 + angleSeparation * i).toDouble())).toFloat() * (mCircleWidth + PADDING_MARKER_PROGRESS_CIRCLE),
                    Math.cos(Math.toRadians(360 - (60 + angleSeparation * i).toDouble())).toFloat() * (mCircleWidth + +PADDING_MARKER_PROGRESS_CIRCLE)
                )
            )
            progressMilestones.add(mMax * (300 - (60 + angleSeparation * i)) / mTotalCircleDegrees)
        }

        /*
        * abs(DEFAULT_END_ANGLE -90)
        * */
        markerPositions.add(
            Pair(
                sin(Math.toRadians((360 - 300).toDouble())).toFloat() * (mCircleWidth + PADDING_MARKER_PROGRESS_CIRCLE),
                cos(Math.toRadians((360 - 300).toDouble())).toFloat() * (mCircleWidth + +PADDING_MARKER_PROGRESS_CIRCLE)
            )
        )
        progressMilestones.add(mMax * (300 - 300) / mTotalCircleDegrees)

        progressMilestones.reverse()
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val padding =
            max(mCircleStrokeWidth / 2f, mPointerStrokeWidth / 2 + mPointerHaloWidth + mPointerHaloBorderWidth)
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        val touchEventRadius =
            Math.sqrt(Math.pow(distanceX.toDouble(), 2.0) + Math.pow(distanceY.toDouble(), 2.0)).toFloat()
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                val pointerRadiusDegrees =
                    (mPointerStrokeWidth * 180 / (Math.PI * Math.max(mCircleHeight, mCircleWidth))).toFloat()
                ...
                if (touchEventRadius >= innerRadius && touchEventRadius <= outerRadius && (cwDistanceFromPointer <= pointerDegrees || ccwDistanceFromPointer <= pointerDegrees)) {
            ...
            MotionEvent.ACTION_MOVE -> if (mUserIsMovingPointer) {
                ...
                cwPointerFromStart = if (cwPointerFromStart < 0) cwPointerFromStart + 360f else cwPointerFromStart
    ...
    private fun init(attrs: AttributeSet?, defStyle: Int) {
        val attrArray = context.obtainStyledAttributes(attrs, R.styleable.cs_CircularSeekBar, defStyle, 0)
    ...
    /**
     * Listener for the KnobView. Implements the same methods as the normal OnSeekBarChangeListener.
     */
    interface OnCircularSeekBarChangeListener {

        fun onProgressChanged(knobView: KnobView, progress: Float, fromUser: Boolean)

        fun onStopTrackingTouch(seekBar: KnobView)

        fun onStartTrackingTouch(seekBar: KnobView)
    }
    ...
    companion object {
         ...
         */
        private val SMALL_DEGREE_BIAS = .1f

        // Default values
        private val DEFAULT_CIRCLE_STYLE = Paint.Cap.ROUND.ordinal
        private val DEFAULT_CIRCLE_X_RADIUS = 30f
        private val DEFAULT_CIRCLE_Y_RADIUS = 30f
        private val DEFAULT_POINTER_STROKE_WIDTH = 14f
        private val DEFAULT_POINTER_HALO_WIDTH = 6f
        private val DEFAULT_POINTER_HALO_BORDER_WIDTH = 0f
        private val DEFAULT_CIRCLE_STROKE_WIDTH = 5f
        private val PADDING_MARKER_PROGRESS_CIRCLE = 20
        private val MARKER_CIRCLE_RADIUS = 5f
        private val DEFAULT_START_ANGLE = -210f // Geometric (clockwise, relative to 3 o'clock)
        private val DEFAULT_END_ANGLE = 30f // Geometric (clockwise, relative to 3 o'clock)
        private val DEFAULT_POINTER_ANGLE = 0f
        private val DEFAULT_MAX = 100
        private val DEFAULT_PROGRESS = 0
        private val DEFAULT_MARKER_STROKE_WIDTH = 3f
        private val DEFAULT_CIRCLE_COLOR = Color.rgb(210, 210, 210)
        private val DEFAULT_MARKER_CIRCLE = Color.rgb(150, 150, 150)
        private val DEFAULT_KNOB_COLOR = Color.rgb(233, 233, 233)
        ...
        private val DEFAULT_CIRCLE_FILL_COLOR = Color.TRANSPARENT
        private val DEFAULT_POINTER_ALPHA = 135
        private val DEFAULT_POINTER_ALPHA_ONTOUCH = 100
        private val DEFAULT_USE_CUSTOM_RADII = false
        private val DEFAULT_MAINTAIN_EQUAL_CIRCLE = true
        private val DEFAULT_MOVE_OUTSIDE_CIRCLE = false
        private val DEFAULT_LOCK_ENABLED = true
        private val DEFAULT_DISABLE_POINTER = false
...
private fun java.util.ArrayList<Float>.findClosest(mProgress: Float): Pair<Int, Float> {
    for (i in 1 until size) {
        if (mProgress >= get(i - 1) && mProgress <= get(i)) {
            if (mProgress == get(i - 1))
                return Pair(i - 1, get(i - 1))
            if (mProgress == get(i))
                return Pair(i, get(i))
            if (abs(mProgress - get(i - 1)) < abs(mProgress - get(i))) {
                return Pair(i - 1, get(i - 1))
            } else {
                return Pair(i, get(i))
            }
